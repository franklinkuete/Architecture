...
{
    public Task<ClientPOCO?> GetClientByIdAsync(int id);
    public Task<IEnumerable<ClientPOCO>> GetAllClientAsync(int pageIndex, int pageSize);
    public Task<ClientPOCO?> AddClientAsync(ClientPOCO client);
    public Task<bool> RemoveClientAsync(int Id);

    public Task<bool> ClientExistsAsync(string firstname, string lastname, DateOnly? dateNaissance);
}
